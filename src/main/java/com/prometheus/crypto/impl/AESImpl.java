...
/**
 * Advanced Encryption Standard (AES) encryption algorithm, implemented by the specification:
...
public class AESImpl {
    ...
    public byte[] decrypt(byte[] message, byte[] key) {
        ...
        state = addRoundKey(state, getRoundKey(keyExpansion,rounds));

        for(int r = 1; r <= rounds; r++) {
            byte[][] roundKey = getRoundKey(keyExpansion,rounds-r);
    ...
    /**
     * Applies Rijandel S-box to every element of the 4x4 state matrix.
     */
    ...
    /**
     * Circular left-shift n-th row vector of the state matrix by n places
     */
    ...
    /**
     * Multiplying matrices in GF(2^8), where each entry of a matrix is
     * represented as a polynomial of rank 7.
     */
    ...
    /**
     * Combines the state matrix with the given round key by xor-ing corresponding entries.
     */
    ...
    /**
     * Expands the encryption key into 4,6 or 8 round keys depending on the key size.
     */
    ...
    /**
     * Performs a cyclic permutation on a given 4-byte word
     */
    ...
    /**
     * Derives a round constant used in key expansion
     */
